...
Enter the observatory office, ask a declared-model question, and collect a traceable branch report. The terminal displays the hosted AI connection status.
...
**Latest experiment: [Recovery Lab — completed, gates failed](https://empirical-observatory.madmanmuzza.chatgpt.site/recovery).** Six models compared the next measured condition in 214 older female mice. Only **3,445 of 4,104 outcomes (83.94%)** were resolvable, below the declared **90%** prerequisite. Added recovery history reduced error by **0.84%**, below the required **5%**, and won against each fold's best comparison in **three of five** folds. These remain descriptive results; they do not demonstrate longer lifespan or causal rejuvenation. [Full result](https://github.com/thantiklermcirony/empirical-observatory/blob/main/public/research/Recovery_Lab_Report.md) · [Frozen source, predictions and independent checks](https://github.com/thantiklermcirony/empirical-observatory/tree/main/research/recovery-lab).
...
The [shared evidence loop](https://github.com/thantiklermcirony/empirical-observatory/blob/main/automation/README.md) now records frozen contracts, source bytes, delayed outcomes and failed checks. Recovery's real retrospective admission record is blocked and contains no forecast rows. A scheduled GB carbon-intensity collector archives official forecasts and later mature estimated actuals on the [public data branch](https://github.com/thantiklermcirony/empirical-observatory/tree/observatory-data), with missing runs and revisions retained. Collection, integrity checks and a deterministic research queue are implemented; automatic model promotion and cross-project policy learning are not. [Operating plan](https://github.com/thantiklermcirony/empirical-observatory/blob/main/public/research/Observatory_Operating_Plan.md).
